-- =====================================================
-- MIGRACIÓN SIMPLE PARA INTEGRACIÓN CON MOODLE
-- =====================================================
...
-- =====================================================
-- AGREGAR CAMPOS A LA TABLA EVENTOS (VERSIÓN SIMPLE)
-- =====================================================

-- Agregar columna moodle_assignment_id si no existe
ALTER TABLE `eventos` ADD COLUMN IF NOT EXISTS `moodle_assignment_id` int NULL;

-- Agregar columna moodle_course_id si no existe  
ALTER TABLE `eventos` ADD COLUMN IF NOT EXISTS `moodle_course_id` int NULL;

-- Agregar índices (MySQL 8.0+ soporta IF NOT EXISTS para índices)
-- Si tu versión de MySQL no soporta esto, puedes comentar estas líneas
-- y ejecutar manualmente si es necesario

-- ALTER TABLE `eventos` ADD INDEX IF NOT EXISTS `moodle_assignment_id` (`moodle_assignment_id`);
-- ALTER TABLE `eventos` ADD INDEX IF NOT EXISTS `moodle_course_id` (`moodle_course_id`);
...
SELECT 'Migración de Moodle completada' as status; 